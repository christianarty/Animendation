...
export function shuffle(
  arr: Array<number> = Array.from({ length: 50 }, (v, i) => i)
): number[] {
  let randomArray = arr;
 ...
 * @param {number} startingNumber - the lower bound of the range for the function
  @returns {number} a single number between startingNumber and endingNumber
*/
export function randomize(
  endingNumber: number,
  startingNumber: number = 0
): number {
  return Math.floor(Math.random() * (endingNumber - startingNumber)) + 1;
}
